This guide explains how to retrieve session data from the Roundtable Proof-of-Human API. There are three primary methods to access your data: by specific session ID, by user ID, or by tag. For more detailed information on the API specification, see the [API Reference](api-reference).
...
> **This route is also available using query parameters**
> `GET https://api.roundtable.ai/v1/tags/sessions?tag={tag}`

### Example request

```bash
curl -X GET https://api.roundtable.ai/v1/tags/checkout-flow/sessions \
  -H "Authorization: Bearer YOUR_SECRET_KEY"
```
...
```bash
curl -X GET "https://api.roundtable.ai/v1/tags/sessions?tag=checkout-flow" \
  -H "Authorization: Bearer YOUR_SECRET_KEY"
```
