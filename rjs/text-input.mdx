...
The TextInput question type inherits methods from the base [Element](custom) class:
...
<ParamField path="validate" type="function">
  ...
  ```javascript
  const isValid = textInputQuestion.validate(true);
  ```
</ParamField>
...
<ParamField path="showValidationError" type="function">
  ...
  ```javascript
  textInputQuestion.showValidationError('Please enter your name.');
  ```
</ParamField>
