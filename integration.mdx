...
<script>
  ...
  window.addEventListener('roundtable:ready', setUid, { once:true }); // Try again once if event fires
</script>
```
The Roundtable Proof-of-Human tracker dispatches a **`roundtable:ready`** event on `window` as soon as it is done loading. Block 2 listens for that completion event so you can safely call `window.setRoundtableUserId` once the tracker is loaded.
...
- Visit your [Account](https://accounts.roundtable.ai/account) to access your data site key.
- Learn about [Accessing Session Data](accessing-session-data) to retrieve risk scores and session data.
- Learn how to [Integrate the Roundtable badge](using-the-badge) on your site
