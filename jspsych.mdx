...
Our tracker adds all data the Alias API needs to trial data with the `jsPsychAliasTracker` extension, stored as `alias_questions`, `alias_responses`, and `alias_question_histories`. This data can be passed to our API without any further modifications. See an example in [call-api.py](https://github.com/roundtableAI/alias-tracker/blob/main/call-api.py).
